...
def single_image_detection(image_path,output_path='detection_output',brightscale=1,batch=False,model_type='vit_h'):
    image_filter = imagefilter(image_path)

    image_filter.bright_image(brightscale)
    image = image_filter.get_image_temp(output_path=output_path, quality=100)

    detection(image,output_path,batch=batch,model_type=model_type)

def multi_image_detection(image_path,output_path='detection_output',brightscale=1,batch=False,model_type='vit_h'):
